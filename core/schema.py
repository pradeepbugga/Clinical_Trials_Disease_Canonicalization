...
def create_canonicalization_tables(cur):
    """
    Create tables used for canonicalizing extracted conditions.
    """

    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS CanonicalConditions (
            canonical_condition_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS CanonicalAbbreviations (
            canonical_condition_id BIGINT NOT NULL,
            abbreviation TEXT NOT NULL,
            PRIMARY KEY (canonical_condition_id, abbreviation),
            FOREIGN KEY (canonical_condition_id)
                REFERENCES CanonicalConditions(canonical_condition_id)
                ON DELETE CASCADE
        )
        """
    )
    ...
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS ExtractedCanonicalConditions (
            extracted_condition_id BIGINT NOT NULL,
            canonical_condition_id BIGINT NOT NULL,
            PRIMARY KEY (
